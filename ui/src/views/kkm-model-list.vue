<template>
  <div>
    ...
    >
      <md-table-toolbar>
        <h1 class="md-title">Модели ККМ</h1>
      ...
      <md-table-empty-state
        ...
        <md-button class="md-primary md-raised" @click="openAddForm">Создать модель ККМ</md-button>
      </md-table-empty-state>
...
export default {
  ...
  computed: {
    ...mapState(["isKkmModelListLoading", "kkmModelListError"]),
    matchedKkmModels: {
      get() {
        return this.$store.getters.matchedKkmModels(this.searchTerm);
      },
      set() {
      }
    },
  ...
  methods: {
    ...
    openAddForm() {
      this.$router.push({name: 'kkm-model-list-item-create'});
    },
    openUpdateForm(id) {
      this.$router.push({name: 'kkm-model-list-item', params: {id: id, action: "update"}});
    },
    openDeleteForm(id) {
      this.$router.push({name: 'kkm-model-list-item', params: {id: id, action: "delete"}});
    },
  }
}
</script>
